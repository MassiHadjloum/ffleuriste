...
function CmdLine(placement, id_cmd, id, bouquet, quantity, prix, date, status) {
    ...
    this.cmd = () => {
        ...
        spand.innerHTML = "traitement déja términer";
        spand.classList = "msgshowdd";
        $('.msgshowd').css({ "position": "absolute", "left": "50%" })
...
$(document).ready(() => {
    ...
    $('#aucp, #aucpp').text("Aucune commande términé");
    //$('#aucpp').text("Aucune commande términé");
    ...
    let myPromise = new Promise((resolve, reject) => {
        $.get('http://localhost:8080/get_Allcommande', (response) => {
            ...
            if (cmds.findIndex((e) => e.status === "traitement términé") !== -1) {
                def = true;
    ...
    myPromise.then((cmds) => {
            ...
            cmds.map((cmd, index) => {
                if (cmd.status === "traitement términé") {
    ...
    let myPromise1 = new Promise((resolve, reject) => {
        $.get('http://localhost:8080/get_Allcommandeperso', (response) => {
            console.log(response)
            let cmds = JSON.parse(response);
            if (cmds.findIndex((e) => e.statue === "traitement términé") !== -1) {
                $(".aucune").prop('hidden', true);
            } else {
    ...
    myPromise1.then((cmds) => {
            cmds.map((cmd, index) => {
                if (cmd.statue === "traitement términé") {
    ...
    /*  let myPromise1 = new Promise((resolve, reject) => {
        $.get('http://localhost:8080/cmd_en_cours', (response) => {
            console.log(response)
            let cmds = JSON.parse(response);
            resolve(cmds);
        })
    ...
    myPromise1.then((cmds) => {
            ...
            cmds.map((cmd, index) => {
                let tr = new CmdLine('perso', cmd.id_client, cmd.bqtprs,
                    '/', cmd.prix, cmd.date, cmd.statue).cmd();
                $('.tbodyp').append(tr);
            });
        })
